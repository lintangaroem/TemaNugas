...
import '../../../screens/group/pages/group_detail_page.dart'; // Akan dibuat nanti
  ...
  Widget _buildGroupListTile(
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          group.description ??
              (isPending ? 'Menunggu persetujuan admin grup' : 'Anggota grup'),
            ...
            isPending
                ...
                : () {
                  ...
                  Navigator.push(
                    ...
                    // Setelah kembali dari GroupDetailPage, refresh data user
                    // untuk memastikan daftar grup di GroupsOverviewPage terupdate
                    // jika ada perubahan (misal, user meninggalkan grup)
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        automaticallyImplyLeading:
            false, // Tidak ada tombol back jika ini root tab
      ),
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            if (user.approvedGroups.isEmpty &&
                user.pendingGroupRequests.isEmpty &&
                user.createdGroups.isEmpty)
              Padding(
            ...
            if (user.createdGroups.isNotEmpty) ...[
              ...
              const SizedBox(height: 8),
              ...user.createdGroups.map(
                (group) => _buildGroupListTile(
                  group,
                  onTap: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Halaman Admin Grup untuk "${group.name}" belum dibuat.',
                        ),
                      ),
                    );
                  },
            ...
            if (user.approvedGroups
                .where((ag) => !user.createdGroups.any((cg) => cg.id == ag.id))
                .isNotEmpty) ...[
              Text(
                "Grup Diikuti",
                style: AppTextStyles.heading.copyWith(fontSize: 18),
              ...
              ...user.approvedGroups
                  .where(
                    (approvedGroup) =>
                        !user.createdGroups.any(
                          (createdGroup) => createdGroup.id == approvedGroup.id,
                        ),
                  )
            ...
            if (user.pendingGroupRequests.isNotEmpty) ...[
              ...
              const SizedBox(height: 8),
              ...user.pendingGroupRequests.map(
                (group) => _buildGroupListTile(group, isPending: true),
              ),
            ],
          ],
        ),
